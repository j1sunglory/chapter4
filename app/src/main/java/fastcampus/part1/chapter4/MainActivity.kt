...
//        }
        ...
        binding.emergencyContactlayer.setOnClickListener{
            ...
            with(Intent(Intent.ACTION_VIEW)) {
                val phoneNumber = binding.mobileValueTextView.text.toString()
                    .replace("-","")
    ...
    private fun getDataUiUpdate() {
        ...
        with(getSharedPreferences(USER_INFORMATION, Context.MODE_PRIVATE)) {
            ...
            binding.birthValueTextView.text = getString(BIRTHDATE,"미입력")
            binding.bloodtypeValueTextView.text = getString(BLOOD_TYPE,"미입력")
            binding.mobileValueTextView.text = getString(EMERGENCY_CONTACT,"미입력")
            val warning = getString(WARNING,"")

            //binding 어쩌구가 너무 반복되고 복잡하니까 A상태면 B 실행해주고, B상태면 A 실행해줘 명령
            binding.textview5.isVisible = !warning.isNullOrEmpty()
            binding.commentValueTextView.isVisible = !warning.isNullOrEmpty()
...
//                binding.commentValueTextView.isVisible = true
                binding.commentValueTextView.text = warning
            }
        }
    }
